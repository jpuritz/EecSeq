...
#BWA for mapping for all samples.  As of version 2.0 can handle SE or PE reads by checking for PE read files
        	for i in "${NAMES[@]}"
        	do
        	if [ -f "$i.R2a.fq.gz" ]; then
        		bwa mem reference.fasta $i.R1a.fq.gz $i.R2a.fq.gz -t $NUMProc -a -M -T 20 -A $optA -B $optB -O $optO -R "@RG\tID:$i\tSM:$i\tPL:Illumina" 2> bwa.$i.log | samtools view -@$NUMProc -q 1 -SbT reference.fasta - > $i.bam 2>$i.bam.log
        	else
...
if [ "$SNP" != "no" ]; then
	...
	if [[ "$MAP" == "no" && ! -f "mapped.bed" ]]; then
		bamToBed -i filter.merged.bam > map.bed
	...
	if [ "filter.merged.bam" -nt "cov.stats" ]; then
		if [ "$BEDTOOLSFLAG" == "OLD" ]; then
			coverageBed -abam filter.merged.bam -b mapped.bed -counts > cov.stats
		else
			bedtools coverage -b filter.merged.bam -a mapped.bed -counts -sorted > cov.stats
		fi
	fi
...
###New implementation of SNP calling here to save on memory	
	...
	freebayes -b split.$1.bam -t mapped.$1.bed -v raw.$1.vcf -f reference.fasta -m 5 -q 5 -E 3 -p 16 -J --min-repeat-entropy 1 -n 20
	rm split.$1.bam*
	}
	export -f call_genos
